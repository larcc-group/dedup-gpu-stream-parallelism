...
sql = """select dataset,mode,threads as threads, (select avg(total_time) from result as result_sequential where   result_sequential.mode = 'sequential') as sequential_time, avg(total_time)  as avg_time, STDEV(total_time) as stdev

 from result where  mode <> 'sequential'
AND mode <> 'spar'
and mode <> 'cuda' and mode <> 'opencl'
AND dataset like "{0}"
...
select dataset,mode,threads  as threads, (select avg(total_time) from result as result_sequential where   result_sequential.mode = 'sequential' ) as sequential_time, avg(total_time)  as avg_time, STDEV(total_time) as stdev
 ...
 order by dataset,threads,mode"""

datasets = [
    {"name":"Silesia", "dataset_name":"%silesia%", "file_name":"silesia"},
    {"name":"Sim Large", "dataset_name":"%large%", "file_name":"large"},
...
for dataset in datasets:
    for mode in ["file"]:
        with DatasetHelper() as db:
            sql_formatted = sql.format(dataset["dataset_name"])
