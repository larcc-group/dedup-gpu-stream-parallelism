...
class DatasetHelper:
    def __init__(self):
        self.database_name = "../benchmark_20190219_complete_with_2xmemory.db"
...
def better_name(filename):
    ...
    filename = filename.replace("spar","SPar")
    filename = filename.replace("cuda","CUDA")
    filename = filename.replace("opencl","OpenCL")
    filename = filename.replace("sequential","Sequencial")

    if filename == "SPar":
        t = str(file_parts[-1])
        filename = filename+" ("+t+" replica"+("s" if int(t) > 1 else "")+")"
    return filename
