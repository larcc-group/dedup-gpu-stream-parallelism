...
void *SerialIntegratedPipelineGpuCuda(struct thread_args targs, int deviceSize)
{
  ...
  [[ spar::ToStream(), spar::Input(temp, chunk, rabintab, rabinwintab, args, preloading_buffer_seek, r, fd, fd_out, currentBatch, deviceIds, deviceSize) ]] while (1)
  {
    ...
    //Move offsets to GPU
    int* d_beginOffsets;
    allocateGpu((void **)&d_beginOffsets, sizeof(int) * beginOffsets.size());
    moveToGpu(d_beginOffsets, beginOffsets.data(), sizeof(int) * beginOffsets.size());
    ...
    [[ spar::Stage(), spar::Input(sha1s, uncompressed_data_gpu, lengths, beginOffsets, chunk, fd, fd_out, maxLength, deviceIdThread, deduplicateStatus,dataCompressed, dataLength,allDataCompressed,d_beginOffsets), spar::Output(sha1s, uncompressed_data_gpu, lengths, beginOffsets, chunk, fd, fd_out, maxLength, deviceIdThread, deduplicateStatus,dataCompressed, dataLength,allDataCompressed,d_beginOffsets), spar::Replicate(deviceSize) ]] {
      ...
      sha1DigestBatchWrapper(uncompressed_data_gpu, lengths1size, d_beginOffsets, lengths1data, sha1s);
    }
