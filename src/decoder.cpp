...
#ifdef ENABLE_LZSS_COMPRESSION
  case COMPRESS_LZSS: {
    int umcompressedSize;
	umcompressedSize = UNCOMPRESS_BOUND;
    r = mbuffer_create(&chunk->uncompressed_data, umcompressedSize);
    if(r != 0) EXIT_TRACE("Creation of decompression buffer failed.\n");
    r = LzssDecodeMemory(chunk->compressed_data.ptr, chunk->compressed_data.n, chunk->uncompressed_data.ptr,UNCOMPRESS_BOUND, &umcompressedSize);
    //TODO: Automatically enlarge buffer if return value is BZ_OUTBUFF_FULL
    if(r!=0) EXIT_TRACE("error uncompressing chunk data\n");
    //Shrink buffer to actual size
    if(umcompressedSize < chunk->uncompressed_data.n) {
      r = mbuffer_realloc(&chunk->uncompressed_data, umcompressedSize);
      assert(r == 0);
    }
    break;
  }
#else
  case COMPRESS_LZSS: {
    EXIT_TRACE("LZSS compression used by input file not supported.\n");
    break;
  }
