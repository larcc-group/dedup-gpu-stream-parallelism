...
void *SerialIntegratedPipelineGpuClSpar(struct thread_args targs, int deviceSize);
void *SerialIntegratedPipelineGpuCl(struct thread_args targs){
  SerialIntegratedPipelineGpuClSpar(targs,getDeviceIds().size());
}
void *SerialIntegratedPipelineGpuClSpar(struct thread_args targs, int deviceSize) {
//   struct thread_args *args = (struct thread_args *)targs;
  ...
  [[spar::ToStream(),spar::Input( temp,chunk, rabintab, rabinwintab, args,preloading_buffer_seek,r,fd, fd_out,currentBatch, deviceIds,context)]]
  while (1) {
    ...
    deviceIdThread = currentBatch % deviceIds.size();
    currentBatch++;
    
    

    cl::CommandQueue queue = getOpenClDefaultCommandQueue(deviceIdThread);
    cl::Buffer uncompressed_data_gpu(context, CL_MEM_READ_ONLY, sizeof(unsigned char) * chunk->uncompressed_data.n);
    ...
    [[spar::Stage(), spar::Input(sha1s, uncompressed_data_gpu,lengths,beginOffsets,chunk,fd, fd_out,context,maxLength,deviceIdThread), spar::Output(sha1s, uncompressed_data_gpu,lengths,beginOffsets,chunk,fd, fd_out,context,maxLength,deviceIdThread) ,spar::Replicate(deviceSize)]]{
      sha1DigestBatchWrapperClWithDevice(uncompressed_data_gpu,lengths.size(), beginOffsets.data(), lengths.data(),sha1s,deviceIdThread);
      
    }

    [[spar::Stage(), spar::Input(sha1s, uncompressed_data_gpu,lengths,beginOffsets,chunk,fd_out,maxLength,r,fd, fd_out,deviceIdThread)]]{
        //Check if is deduplicated
        cl_buffer_region region;
        cl::Buffer subInput;
        int n = maxLength + (maxLength / 8) + 1000;
              ...
              switch(conf->compress_type){
                case COMPRESS_LZSS:
                  ...
                  if(lengths[i] < 1024){
                    ...
                    // EXIT_TRACE("No lzss support on opencl yet...");
                    

                    //Create subbuffer based on length and offset
                    // int begin = 1;
                    region = {beginOffsets[i],lengths[i] * sizeof(unsigned char)};
                    subInput = uncompressed_data_gpu.createSubBuffer(CL_MEM_READ_ONLY, CL_BUFFER_CREATE_TYPE_REGION,&region);
                    r = LzssEncodeMemoryGpu((unsigned char*)(chunk->uncompressed_data.ptr + beginOffsets[i]), subInput, lengths[i], compressedData, n, &compressedSize, deviceIdThread);
