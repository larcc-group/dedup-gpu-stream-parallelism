...
//       int offset = beginOffsets[i];
//       int len = lengths[i];
//       SHA1_Digest(chunk->uncompressed_data.ptr + offset, len, sha1s + i * SHA1_LEN);
//     }


    //Testing
    // unsigned char * sha1sGold = new unsigned char [lengths.size() * SHA1_LEN];//GPU friendly way of holding sha1
    
    // for(int i = 0; i < lengths.size(); i++){
    //   int offset = beginOffsets[i];
    //   int len = lengths[i];
    //   unsigned char hash[SHA1_LEN];
    //   SHA1_Digest(chunk->uncompressed_data.ptr + offset, len, hash);
    //   memcpy(sha1sGold + i * SHA1_LEN, hash, SHA1_LEN);
    //   if(i == 1){
    //         printf("CPU - Begin: %i, length: %i\n",offset,len);
    ...
    // for(int i = 0; i < lengths.size(); i++){
    //   if(memcmp(sha1sGold + (i * SHA1_LEN), sha1s + (i * SHA1_LEN), SHA1_LEN) != 0){
        
    //     printf("Differences found at %i, expected:\n",i);
    //     for(int j=0;j<SHA1_LEN;j++)
    //       printf("%02x", sha1sGold[i * SHA1_LEN + j]);
    //     printf("\ngot:\n");
    //     for(int  j=0;j<SHA1_LEN;j++)
    //       printf("%02x", sha1s[i * SHA1_LEN + j]);
    //     printf("\n");
