...
void *spar_call(void * targs, int NTHREADS) {
    ...
    while (1) {
        ...
        [[spar::ToStream,spar::Input(rabintab,rabinwintab,NTHREADS)]]do {
            ...
            //printf("chunck-size: %d - offset: %d\n", chunk->uncompressed_data.n, offset);
            if(offset == 0) {
                ...
                if(true) { //Necessary to bypass a SPar syntax limitation.
                    [[spar::Stage,spar::Input(chunk),spar::Output(chunk),spar::Replicate(NTHREADS)]] {
...
void EncodeSPar(config_t * _conf) {
    ...
    int milliseconds = (((stoptime.tv_sec - starttime.tv_sec) * 1000000) + 
            (stoptime.tv_usec - starttime.tv_usec))/1000;
    // double secs = (double)(stoptime.tv_usec - starttime.tv_usec) / 1000000 + (double)(stoptime.tv_sec - starttime.tv_sec);
    printf("It took %i\n", milliseconds);
