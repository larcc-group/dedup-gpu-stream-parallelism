...
void *SerialIntegratedPipelineGpuOpenCl(struct thread_args targs, int deviceSize)
{
  ...
  [[ spar::ToStream(), spar::Input(temp, chunk, rabintab, rabinwintab, args, preloading_buffer_seek, r, fd, fd_out, currentBatch, deviceIds, deviceSize) ]] 
  while (1)
  {
        ...
        #ifndef OPENCL_C
        cl::Context context = getOpenClDefaultContext();

        cl::CommandQueue queue = getOpenClDefaultCommandQueue(deviceIdThread);
        cl::Buffer uncompressed_data_gpu(context, CL_MEM_WRITE_ONLY, sizeof(unsigned char) * chunk->uncompressed_data.n);
        queue.enqueueWriteBuffer(uncompressed_data_gpu, CL_FALSE, 0, sizeof(unsigned char) * chunk->uncompressed_data.n, chunk->uncompressed_data.ptr);
        

        
        cl::Buffer d_beginOffsets(context, CL_MEM_WRITE_ONLY, sizeof(int) * beginOffsets.size());
        queue.enqueueWriteBuffer(d_beginOffsets, CL_FALSE, 0, sizeof(int) * beginOffsets.size(), beginOffsets.data());
        
        queue.finish();
        #else
         cl_context context = getOpenClDefaultContextC();

        cl_command_queue queue = getOpenClDefaultCommandQueueC(deviceIdThread);
        cl_int err;
        
        cl_mem uncompressed_data_gpu = clCreateBuffer(context, CL_MEM_WRITE_ONLY,sizeof(unsigned char) * chunk->uncompressed_data.n, NULL, &err);
        OpenCLCheckError(err);
        OpenCLCheckError( clEnqueueWriteBuffer(queue, uncompressed_data_gpu, CL_FALSE, 0, sizeof(unsigned char) * chunk->uncompressed_data.n, chunk->uncompressed_data.ptr, 1, NULL, NULL) );

        cl_mem d_beginOffsets = clCreateBuffer(context, CL_MEM_WRITE_ONLY,sizeof(int) * beginOffsets.size(), NULL, &err);
        OpenCLCheckError(err);
        cl_event evt_write;
        OpenCLCheckError( clEnqueueWriteBuffer(queue, d_beginOffsets, CL_FALSE, 0, sizeof(int) * beginOffsets.size(), beginOffsets.data(), 1, NULL, &evt_write) );


        OpenCLCheckError( clWaitForEvents(1, &evt_write) );
        ...
        [[ spar::Stage(), spar::Input(sha1s, uncompressed_data_gpu, lengths, beginOffsets, chunk, fd, fd_out, maxLength, deviceIdThread, deduplicateStatus, dataCompressed, dataLength,allDataCompressed, d_beginOffsets), spar::Output(sha1s, uncompressed_data_gpu, lengths, beginOffsets, chunk, fd, fd_out, maxLength, deviceIdThread, deduplicateStatus, dataCompressed, dataLength,allDataCompressed, d_beginOffsets), spar::Replicate(deviceSize) ]] 
        {
          // auto beginOffsets1 = beginOffsets.data();
          auto lengths1data = lengths.data();
          auto lengths1size = lengths.size();
          sha1DigestBatchWrapperClWithDevice(uncompressed_data_gpu, lengths1size, d_beginOffsets, lengths1data, sha1s, deviceIdThread);
        }

        [[ spar::Stage(), spar::Input(sha1s, uncompressed_data_gpu, lengths, beginOffsets, chunk, fd_out, maxLength, r, fd, fd_out, deviceIdThread, deduplicateStatus, dataCompressed, dataLength,allDataCompressed, d_beginOffsets), spar::Output(sha1s, uncompressed_data_gpu, lengths, beginOffsets, chunk, fd, fd_out, maxLength, deviceIdThread, deduplicateStatus, dataCompressed, dataLength,allDataCompressed, d_beginOffsets) ]]
        {
        ...
        [[ spar::Stage(), spar::Input(sha1s, uncompressed_data_gpu, lengths, beginOffsets, chunk, fd, fd_out, maxLength, deviceIdThread, deduplicateStatus, dataCompressed, dataLength,allDataCompressed, d_beginOffsets), spar::Output(sha1s, uncompressed_data_gpu, lengths, beginOffsets, chunk, fd, fd_out, maxLength, deviceIdThread, deduplicateStatus, dataCompressed, dataLength,allDataCompressed, d_beginOffsets), spar::Replicate(deviceSize) ]] 
        {
          ...
          LzssEncodeMemoryGpuBatch(chunk->uncompressed_data.ptr,uncompressed_data_gpu,chunk->uncompressed_data.n,beginOffsets.data(),d_beginOffsets,beginOffsets.size(),&allDataCompressed,deviceIdThread);
        }

        [[ spar::Stage(), spar::Input(sha1s, uncompressed_data_gpu, lengths, beginOffsets, chunk, fd, fd_out, maxLength, deviceIdThread, deduplicateStatus, dataCompressed, dataLength,allDataCompressed, d_beginOffsets) ]]
        {
          for (int i = 0; i < lengths.size(); i++)
          {
            int r;
