...
void *SerialIntegratedPipelineGpuCuda(struct thread_args targs, int deviceSize)
{
  ...
  [[ spar::ToStream(), spar::Input(temp, chunk, rabintab, rabinwintab, args, preloading_buffer_seek, r, fd, fd_out,currentBatch, deviceIds,deviceSize) ]] while (1)
  {
    ...
    [[ spar::Stage(), spar::Input(sha1s, uncompressed_data_gpu, lengths, beginOffsets, chunk, fd, fd_out, maxLength,deviceIdThread), spar::Output(sha1s, uncompressed_data_gpu, lengths, beginOffsets, chunk, fd, fd_out, maxLength,deviceIdThread), spar::Replicate(deviceSize) ]]
    {
    setDevice(deviceIdThread);
      ...
      for (int i = 0; i < lengths.size(); i++)
      {
        ...
        else
        {
          ...
          case COMPRESS_LZSS:
            ...
            {
              r = LzssEncodeMemoryGpu((unsigned char *)(chunk->uncompressed_data.ptr + beginOffsets[i]), uncompressed_data_gpu + beginOffsets[i], lengths[i], compressedData, n, &compressedSize, deviceIdThread);
            }
            if (r != 0)
            {
