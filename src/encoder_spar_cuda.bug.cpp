...
void *SerialIntegratedPipelineGpuCuda(struct thread_args targs, int deviceSize)
{
  ...
  [[ spar::ToStream(), spar::Input(temp, chunk, rabintab, rabinwintab, args, preloading_buffer_seek, r, fd, fd_out, currentBatch, deviceIds, deviceSize) ]] while (1)
  {
    ...
    [[ spar::Stage(), spar::Input(sha1s, uncompressed_data_gpu, lengths, beginOffsets, chunk, fd, fd_out, maxLength, deviceIdThread, deduplicateStatus,data, dataLength), spar::Output(sha1s, uncompressed_data_gpu, lengths, beginOffsets, chunk, fd, fd_out, maxLength, deviceIdThread, deduplicateStatus,data, dataLength) ]] {
      setDevice(deviceIdThread);
      ...
      printf("a");
      sha1DigestBatchWrapper(uncompressed_data_gpu, lengths1size, beginOffsets1, lengths1data, sha1s);
    }

        [[ spar::Stage(), spar::Input(sha1s, uncompressed_data_gpu, lengths, beginOffsets, chunk, fd_out, maxLength, r, fd, fd_out, deviceIdThread, deduplicateStatus,data, dataLength), spar::Output(sha1s, uncompressed_data_gpu, lengths, beginOffsets, chunk, fd, fd_out, maxLength, deviceIdThread, deduplicateStatus,data, dataLength) ]]
    {
      ...
      for (int i = 0; i < lengths.size(); i++)
      {
        ...
        if (!isDeduplicate)
        {
          ...
          case COMPRESS_LZSS:
            ...
            if (lengths[i] < 1024)
            {
              r = LzssEncodeMemory((unsigned char *)(chunk->uncompressed_data.ptr + beginOffsets[i]), lengths[i], compressedDataTmp, n, &compressedSize);
            }
            else
            {
              r = LzssEncodeMemoryGpu((unsigned char *)(chunk->uncompressed_data.ptr + beginOffsets[i]), uncompressed_data_gpu + beginOffsets[i], lengths[i], compressedDataTmp, n, &compressedSize, deviceIdThread);
            }
            if (r != 0)
            {
            ...
            data.push_back(compressedData);
            dataLength.push_back(compressedSize);
            break;
          case COMPRESS_NONE:
            ...
            data.push_back((unsigned char *)(chunk->uncompressed_data.ptr + beginOffsets[i]));
            dataLength.push_back(lengths[i]);
            break;
          default:
        ...
        [[ spar::Stage(), spar::Input(sha1s, uncompressed_data_gpu, lengths, beginOffsets, chunk, fd, fd_out, maxLength, deviceIdThread, deduplicateStatus,data, dataLength) ]]
    {
      ...
      for (int i = 0; i < lengths.size(); i++)
      {
        ...
        {
          r = write_file(fd_out, (unsigned char)TYPE_COMPRESS, dataLength[currentDataIndex], data[currentDataIndex]);
          if (r != 0)
          {
            EXIT_TRACE("Error in write_file %i", r);
          }
