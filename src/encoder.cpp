...
void sub_Compress(chunk_t *chunk) {
    ...
    switch (conf->compress_type) {
      ...
      case COMPRESS_LZSS:
        ...
        //Lzss compression buffer must be at least N + N/8
        n = chunk->uncompressed_data.n + (chunk->uncompressed_data.n / 8) + 100;
        r = mbuffer_create(&chunk->compressed_data, n);
        if(r != 0) {
          EXIT_TRACE("Creation of compression buffer failed.\n");
        }
        //compress the block
		    int lzssCompressedSize;
        lzssCompressedSize = n;
        r = LzssEncodeMemory(chunk->uncompressed_data.ptr, chunk->uncompressed_data.n,chunk->compressed_data.ptr, n,&lzssCompressedSize);
              // printf("Compressed data by %i %i\n",chunk->uncompressed_data.n,lzssCompressedSize);
       
        if (r != 0) {
		      EXIT_TRACE("Compression failed\n");
        }
		 n = lzssCompressedSize;
        //Shrink buffer to actual size
        if(n < chunk->compressed_data.n) {
          r = mbuffer_realloc(&chunk->compressed_data, n);
          assert(r == 0);
...
#ifdef PRINT_HASHES
    unsigned char* hash = (unsigned char *)(chunk->sha1);
...
void Encode(config_t * _conf) {
  ...
  #ifdef ENABLE_SERIAL_GPU
    #ifdef USE_OPENCL
      SerialIntegratedPipelineGpuCl(generic_args);
    #else
      SerialIntegratedPipelineGpu(generic_args);
    #endif
  #else
  //Do the processing
  SerialIntegratedPipeline((void*)&generic_args);
  #endif
  std::chrono::steady_clock::time_point end = std::chrono::steady_clock::now();
  long totalMilliseconds = std::chrono::duration_cast<std::chrono::milliseconds>(end - begin).count();
  printf("It took %i\n",totalMilliseconds);
#ifdef ENABLE_PARSEC_HOOKS
  __parsec_roi_end();
#endif
